...
| `ydk_dialog.py` | `YdkImportDialog`: griglia **carta → stampe** a fisarmonica (`QTreeWidget`: espansione, scorrimento e tastiera gratis su ~800 righe, con `setUniformRowHeights` — senza, Qt misura ogni riga). **Niente è preselezionato, per principio**: in un `.ydk` la rarità non c'è e Ash Blossom ha 36 stampe, quindi sceglierne una sarebbe inventare (stessa regola del ponte dal Database). Le carte senza stampa scelta NON entrano nella base e il riepilogo lo dice. `sort_printings` ordina per `rarity.rarity_rank` (comuni prima; rank -1 = sconosciute tutte in fondo), poi per set. **Due stampe con rarità ed espansione identiche non si fondono**: sono blueprint diversi con prezzi propri (**152 gruppi su 47.951 combinazioni**, misurato sul catalogo vero), e si distinguono aggiungendo `#<ref_id>`. **Niente miniature, di proposito**: sarebbero ~40 immagini scaricate all'apertura, cioè la raffica verso il CDN che il progetto vieta; le pillole di rarità e i set si disegnano in locale. **`setIconSize` è obbligatorio**: senza, l'albero scala le pillole a 16px e la sigla della rarità non si legge più. I pulsanti standard di `QDialogButtonBox` li traduce Qt, non noi: in italiano "Cancel" resta inglese, quindi il testo si scrive a mano. |
