...
| `ydk.py` | Lettura dei file **`.ydk`** (lo scambio di mazzi fra siti e programmi): testo → `YdkDeck`/`YdkCard`, **senza Qt, senza rete, senza database**, così lo smoke test lo prova per intero. Formato: una riga per COPIA col *passcode*, sezioni `#main`/`#extra` e `!side` (l'asimmetria cancelletto/esclamativo è del formato, non un errore; si accetta anche `#side`). **Le tre sezioni si SOMMANO** in `total`: una base serve a sapere quante copie *possedere*, e nel file d'esempio *Mulcharmy Purulia* sta 2 nel main e 1 nel side. Le quantità per sezione restano, e `sections_label()` le mostra **solo** se la carta è divisa (`split`), altrimenti il numero basta. Difensivo per scelta: numeri prima di qualunque intestazione finiscono nel main invece di perdersi, gli zeri davanti si normalizzano con `int()`, e le righe non capite **non si buttano** — vanno in `ignored` col numero di riga e l'interfaccia le mostra. `decode()` prova utf-8-sig/utf-8/cp1252 e ripiega su latin-1, che non fallisce mai: contano solo le cifre. |
| `ydk_dialog.py` | `YdkImportDialog`: a sinistra il **mazzo come griglia di immagini** (`QListWidget` in `IconMode`, celle 118×214 — con 132 di larghezza ci stavano solo 4 carte per riga, con 118 ne stanno 6), a destra le **stampe della carta selezionata**. **Niente è preselezionato, per principio**: in un `.ydk` la rarità non c'è e Ash Blossom ha 36 stampe, quindi sceglierne una sarebbe inventare (stessa regola del ponte dal Database). Le carte senza stampa scelta NON entrano nella base e il riepilogo lo dice. `sort_printings` ordina per `rarity.rarity_rank` (comuni prima; rank -1 = sconosciute tutte in fondo), poi per set. **Due stampe con rarità ed espansione identiche non si fondono**: sono blueprint diversi con prezzi propri (**152 gruppi su 47.951 combinazioni**, misurato sul catalogo vero). **Immagini da `core.card_images`**, cioè dalla cache su DISCO condivisa col Database: un mazzo è un insieme CHIUSO (~40-60 carte), quindi si chiedono tutte e non solo quelle a schermo — misurato: 39 immagini in **5 s** la prima volta, **0 s** e zero rete la seconda. Tre trappole viste a schermo, non supposte: **`setIconSize` è obbligatorio** o le pillole di rarità scendono a 16px e la sigla non si legge; i nomi delle espansioni sono lunghissimi e senza `setTextElideMode` venivano **tagliati di netto** con una barra orizzontale, quindi il **codice del set sta davanti** (in coda i puntini se lo mangiavano) e con esso il `#<ref_id>` che distingue i doppioni; i pulsanti standard di `QDialogButtonBox` li traduce Qt, non noi, e in italiano "Cancel" restava inglese. **Esclusione col clic destro (v1.6.0):** `customContextMenuRequested` → `itemAt(pos)` → interruttore su `_excluded` (nessun menù: il gesto È il comando). Una carta esclusa è rossa e **non entra nella base**, qualunque stampa le fosse stata scelta — ma la scelta **non si cancella**, così rimetterla dentro la ritrova: escludere è un ripensamento, non un azzeramento. Nei conteggi le escluse si sottraggono da tutti gli altri totali e si dicono a parte ("3 carte escluse" ≠ "3 senza stampa": la prima è una scelta, la seconda una dimenticanza, e sommarle direbbe due volte la stessa carta). Il pulsante si abilita sulle scelte **non escluse**, altrimenti escludere l'ultima carta scelta creerebbe una base vuota. **La selezione (v1.5.3)** sta nel tema come `QListWidget#deckGrid`, non in un foglio locale. Tre cose, tutte viste a schermo: la palette dà come evidenziazione il **teal pieno**, che su una griglia di illustrazioni diventa un rettangolo netto — si usa il **teal traslucido** già in uso nella tabella principale (`rgba(26,195,178,0.14)`) più un filo di bordo; **ogni cella è una piastrella** (`background` anche da spenta), perché con le celle trasparenti la selezione sembrava una macchia invece di una variazione; e il **bordo c'è sempre**, trasparente da spento, altrimenti comparendo solo da selezionato sposterebbe la cella di un pixel a ogni clic. In più `_icon_for` registra lo stesso pixmap per `Normal`, `Selected` e `Active`: di suo Qt ridisegna l'icona in modalità `Selected` stendendoci sopra una velatura del colore di evidenziazione, e su un'illustrazione si vede come uno sbiadimento. |
...
| *(immagini)* | Spostato in **`core/card_images.py`** nella v1.5.1: la stessa cache la usa la griglia dell'importazione `.ydk` del Market Watch. |
...
- **GOTCHA 28 — sull'elemento di una lista il QSS batte `setBackground`, ma
  non `setForeground`** (v1.6.0).
    - **Sintomo:** `item.setBackground(QColor(...))` su una cella di
      `QListWidget` non si vede. `item.setForeground(...)` invece sì — quindi
      il testo cambia colore e lo sfondo no, che a occhio sembra un difetto
      del disegno.
    - **Causa:** il tema dà lo sfondo con `QListWidget#deckGrid::item {
      background: … }`, e per lo sfondo il foglio di stile ha la precedenza
      sul dato dell'elemento. Il colore del testo, in quel blocco, non è
      dichiarato: lì passa il dato.
    - **Cura:** quando serve uno stato per-cella che il QSS non conosce (qui:
      la carta **esclusa** in rosso), lo si **dipinge nel pixmap** dell'icona
      — `_paint_excluded` in `ydk_dialog.py`: velo translucido, cornice e una
      ✕ nell'angolo. È l'unica via che non dipende da chi vince fra foglio di
      stile e dato dell'elemento. Il velo resta **translucido** di proposito:
      serve capire *quale* carta si è esclusa, non solo che ce n'è una.
...
- **L'eccezione al ponte: il market_watch legge `cdb_cards`** (v1.5.0,
  importazione `.ydk`). È l'unica lettura fuori dal proprio prefisso in tutto
  il progetto, ed è deliberata. Un `.ydk` porta **passcode**; il catalogo
  prezzi conosce solo **nomi**; la traduzione esiste solo nella tabella del
  Database. Le alternative sono peggiori: duplicare 14.000 righe sotto `mw_`
  vuol dire una seconda sincronizzazione da tenere allineata, e un servizio di
  ...
  **neppure su YGOPRODeck**, che risponde 400.
