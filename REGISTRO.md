...
82. **Escludere una carta col tasto destro (v1.6.0).** Un mazzo scaricato non
    è una lista della spesa: le tre Ash Blossom uno le ha già, e tenerle nella
    base gonfia il totale con soldi che non spenderà. Su richiesta dell'utente,
    ora nella griglia dell'importazione il **tasto destro** su una carta la
    **esclude**: diventa rossa — velo, cornice e una ✕ nell'angolo — e non
    entra nella base, anche se le avevi già scelto una stampa.
    Due scelte fatte di proposito. La stampa scelta **non si cancella**: se
    cambi idea e la rimetti dentro (altro clic destro) la ritrova, perché
    escludere è un ripensamento, non un azzeramento. E nel riepilogo le escluse
    si contano **a parte**: "3 carte escluse" è un'altra cosa da "3 senza
    stampa" — la prima è una decisione, la seconda una dimenticanza, e
    sommarle direbbe due volte la stessa carta.
    Il velo rosso è translucido di proposito: la carta deve restare
    riconoscibile, perché serve sapere *quale* hai escluso. E il pulsante per
